...
import { scrapeTechNews } from './crawler.service';
...
// 触发爬虫任务
export const triggerCrawlerTask = async (task: any) => {
  try {
...
// 启动定时任务
export const startScheduledTask = (task: any) => {
  // 如果任务已存在，先停止
  if (scheduledTasks.has(task._id.toString())) {
    stopScheduledTask(task._id.toString());
  }
...
// 停止定时任务
export const stopScheduledTask = (taskId: string) => {
